        ...
        if coro is None:
            ...
            folium.GeoJson(
                ...
                style_function=lambda feature: {
                    'fillColor': colormap(feature['properties'].get('Pob_Exp', 0)),
                    'color': '#8895a8', 'weight': 0.5, 'fillOpacity': 0.7
                },
                tooltip=folium.GeoJsonTooltip(
                    fields=['ADM1_NAME', 'Pob_Exp'] if 'ADM1_NAME' in g_dep_temp.columns else ['Pob_Exp'], 
    ...
    if capa:
        ...
        if os.path.exists(p):
            ...
            folium.raster_layers.ImageOverlay(
                ...
                name=capa, interactive=False, cross_origin=False, zindex=1,
            ).add_to(m)
            m.fit_bounds(bounds)
        else:
...
def sec_inicio():
    st.markdown('<div class="hero">'
      '<h1 style="color:#ffffff !important;">Terremoto de Colombia M7.4</h1>'
...
def sec_sismo():
    ...
    if sint:
        st.warning('Catálogo ilustrativo (Omori–GR): la API del USGS aún no publica réplicas.')
...
def sec_comparativa():
    ...
    a, b = st.columns(2)
    with a:
        st.markdown('<div class="card card-suelo"><h3>🏜️ Cuenca y topografía</h3><ul class="mini"><li>El contraste de rigidez de los estratos refleja, refracta y filtra las ondas.</li><li><b>Depósitos blandos:</b> posible amplificación y mayor duración.</li><li><b>Cuencas:</b> reflejo y atrapamiento de ondas.</li><li><b>Relieves:</b> concentración o dispersión según su geometría.</li></ul></div>', unsafe_allow_html=True)
    with b:
        st.markdown('<div class="card card-suelo"><h3>💦 Licuación de suelos</h3><ul class="mini"><li>Pérdida súbita de resistencia del terreno.</li><li>Requiere: suelo granular suelto + saturación de agua + demanda cíclica fuerte.</li><li>Las estructuras pueden hundirse o inclinarse por pérdida de soporte.</li></ul></div>', unsafe_allow_html=True)
        
    st.subheader('Cada estructura "escucha" un sismo diferente')
    a, b, c = st.columns(3)
    with a:
        st.markdown('<div class="card card-col"><h3>🏠 Bajas (1–3 pisos)</h3><p class="mini">Responden con mayor fuerza a ondas de periodo corto (alta frecuencia, ~0.3 s).</p></div>', unsafe_allow_html=True)
    with b:
        st.markdown('<div class="card card-col"><h3>🏢 Medianas</h3><p class="mini">Más sensibles a ondas de periodo intermedio (~1.0 s).</p></div>', unsafe_allow_html=True)
    with c:
        st.markdown('<div class="card card-col"><h3>🏙️ Altas</h3><p class="mini">Entran en resonancia con ondas de periodo largo (baja frecuencia, ~3.0 s).</p></div>', unsafe_allow_html=True)
...
def sec_intensidad():
    ...
    for i, (num, col, nom, desc) in enumerate(MMI):
        with cols[i]:
            st.markdown('<div class="mmi-chip" style="background:' + col + '"><span class="num">' + num + '</span><b>' + nom + '</b><br>' + desc + '</div>', unsafe_allow_html=True)
    st.markdown('---')
    mapa_interactivo('Intensidad (MMI)',
      capa='intensity_overlay.png', bb=BINT,
      items=[(x[1], x[0] + ' ' + x[2]) for x in MMI],
...
def sec_edificaciones():
    ...
    if not d_ciu.empty:
        ...
        defs = [o for o in ['Cali', 'Pereira', 'Manizales', 'Bogota'] if o in opts]
...
def sec_secundarias():
    ...
    if capa == 'desliz.png':
        items = [('#ffffb2', 'Baja'), ('#fd8d3c', 'Media'), ('#bd0026', 'Alta')]
        nota = 'Susceptibilidad = PGA × pendiente, calibrada en zona sacudida'
    elif capa == 'liq.png':
        items = [('#deebf7', 'Baja'), ('#6baed6', 'Media'), ('#08519c', 'Alta')]
        nota = 'Licuefacción: valles planos y húmedos con sacudida fuerte'
    else:
...
def sec_validacion():
    ...
    if d_est.empty:
        ...
        with a:
            ...
            mn = max(0.01, float(min(d_est.pga_obs.min(), d_est.pga_mod.min())))
...
def sec_metodologia():
    ...
    if os.path.exists(D):
        for f in sorted(os.listdir(D)):
            if f.endswith('.csv'):
                try:
                    st.download_button(f, open(f'{D}/{f}', 'rb').read(), file_name=f, key=f)
                except Exception:
                    pass
...
SECCIONES = ['🏠 Inicio', '🌍 El sismo', '🆚 Colombia vs Venezuela', '🎯 Intensidad (MMI)', '👥 Población expuesta', '🏗️ Edificaciones', '⛰️ Amenazas secundarias', '✅ Validación', '📚 Aprende', '🔬 Metodología y datos']
...
RUTAS = {
  '🏠 Inicio': sec_inicio, '🌍 El sismo': sec_sismo, '🆚 Colombia vs Venezuela': sec_comparativa,
  '🎯 Intensidad (MMI)': sec_intensidad, '👥 Población expuesta': sec_poblacion, '🏗️ Edificaciones': sec_edificaciones,
  '⛰️ Amenazas secundarias': sec_secundarias, '✅ Validación': sec_validacion, '📚 Aprende': sec_aprende,
